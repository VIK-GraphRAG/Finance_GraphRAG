        ...
        if st.button("🔐 Process PDF (Local Only)", type="primary"):
            with st.spinner("🔒 로컬 모델로 처리 중... (외부 전송 없음)"):
                try:
                    ...
                    if response.status_code == 200:
                        data = response.json()
                        st.success("✅ PDF가 로컬 모델로 안전하게 처리되었습니다!")
    ...
    if "CSV" in upload_type:
        ...
        if csv_file:
            ...
            try:
                ...
                with col1:
                    entity_col = st.selectbox("엔티티 이름 컬럼", df.columns, help="회사명, 인물명 등")
                    entity_type = st.selectbox("엔티티 타입", ["Company", "Person", "Technology", "Country"], help="노드 타입")
                
                with col2:
                    property_cols = st.multiselect("속성 컬럼", df.columns, help="Revenue, MarketCap 등")
