...
# 프로젝트 루트 경로 설정 (동적으로 찾기)
PROJECT_ROOT = Path(__file__).parent.parent.absolute()
DEBUG_LOG_PATH = PROJECT_ROOT / '.cursor' / 'debug.log'
# .cursor 디렉토리가 없으면 생성
DEBUG_LOG_PATH.parent.mkdir(parents=True, exist_ok=True)
ERROR_LOG_PATH = PROJECT_ROOT / 'drone_error.log'
ERROR_LOG_PATH.parent.mkdir(parents=True, exist_ok=True)
ERROR_LOG_PATH.touch(exist_ok=True)

# 프로젝트 루트를 Python path에 추가 (batch_upload 등 사용)
if str(PROJECT_ROOT) not in sys.path:
    sys.path.insert(0, str(PROJECT_ROOT))


def droneLogError(errorSource: str, errorMessage: str) -> None:
    """
    Log error details to drone_error.log
    """
    try:
        timestamp = __import__('time').time()
        with open(ERROR_LOG_PATH, 'a') as logFile:
            logFile.write(f"{timestamp} | {errorSource} | {errorMessage}\n")
    except Exception:
        pass
...
# --- [11.5] Multi-Hop Query with Fallback ---
@app.post("/query_multihop",
          summary="Multi-Hop Reasoning Query",
          description="Query with multi-hop reasoning + Perplexity fallback")
async def queryMultihop(request: QueryRequest):
    """
    멀티홉 추론 + Perplexity 폴백 쿼리
    
    처리 순서:
    1. Neo4j Multi-Hop Reasoning (영구 DB + 임시 PDF 데이터)
    2. 신뢰도 < 60% → Perplexity API 폴백
    3. Perplexity 실패 → 낮은 신뢰도 결과라도 반환
    """
    try:
        print(f"\n{'='*70}")
        print(f"Multi-Hop Query: {request.question}")
        print(f"{'='*70}")
        ...
        # 멀티홉 추론 + 폴백 처리
        result = await processor.processQuery(
            question=request.question,
            max_hops=3,
            use_fallback=request.enable_web_search
        )
        
        # 결과 포맷팅
        if result["source"] in ["neo4j_multihop", "neo4j_multihop_low_confidence"]:
            # Neo4j 멀티홉 결과
            sources_list = []
            for i, step in enumerate(result.get("reasoning_path", []), 1):
                sources_list.append({
                    'id': i,
                    'file': 'Neo4j Knowledge Graph',
                    'type': 'reasoning_step',
        ...
        elif result["source"] == "perplexity_api":
            # Perplexity 폴백 결과
            sources_list = []
            ...
            if "neo4j_context" in result:
                sources_list.append({
                    ...
                    'file': 'Neo4j Context (Low Confidence)',
                    'type': 'graph_context',
                    'content': result["neo4j_context"]["answer"],
                    'confidence': result["neo4j_context"]["confidence"]
                })
            ...
            return {
                ...
                "fallback_used": True,
                "neo4j_context": result.get("neo4j_context"),
                "status": "success"
            }
        ...
        droneLogError("query_multihop", str(e))
        raise HTTPException(status_code=500, detail=f"Multi-hop query failed: {str(e)}")


# --- [11.7] Base Data Loader ---
@app.post("/load_base_data",
          summary="Load base data folder into Neo4j",
          description="Load all files under Finance_GraphRAG/data into the permanent graph")
async def loadBaseData(
    resetDb: bool = Form(False),
    folderPath: str = Form(None)
):
    """
    Load base data folder into Neo4j as permanent graph
    """
    try:
        # scripts 폴더에서 batch_upload 가져오기
        ...
        data_path = folderPath or str(PROJECT_ROOT / "data" / "baseline")

        if resetDb and neo4j_db:
        ...
        uploader.saveReport(output_file="upload_report.json")
        ...
        droneLogError("load_base_data", str(e))
        raise HTTPException(status_code=500, detail=f"Base data load failed: {str(e)}")


# --- [12] PDF Upload Endpoint ---
@app.post("/ingest_pdf",
          summary="PDF Upload and Processing (Temporary)",
          description="Upload PDF document for temporary processing - entities merged into existing graph")
async def ingest_pdf(file: UploadFile = File(...), permanent: bool = Form(False)):
    """
    Upload and process PDF with local security model
    
    Mode:
    - permanent=False (default): Temporary processing, entities added to session graph
    - permanent=True: Save to permanent database
    ...
    try:
        ...
        print(f"📄 Received PDF upload: {file.filename} (permanent={permanent})")
        ...
        storage_type = "permanent" if permanent else "temporary"
        
        if neo4j_db:
            if not permanent:
                # Temporary mode: clear previous temporary overlay
                cleanup_query = "MATCH (n) WHERE n.temporary = true DETACH DELETE n"
                neo4j_db.execute_query(cleanup_query)
            ...
            print(f"💾 Storing {len(entities_for_neo4j)} entities in Neo4j ({storage_type})...")
            integrator.ingestPdfEntities(entities_for_neo4j, permanent=permanent)
            
            print(f"✅ PDF processing complete ({storage_type})")
        else:
        ...
        return {
            "message": f"PDF processed successfully ({storage_type})",
        ...
        droneLogError("ingest_pdf_import", str(e))
        raise HTTPException(
            status_code=500,
            detail=f"Missing required package: {str(e)}"
        )
    except Exception as e:
        import traceback
        ...
        print(error_trace)
        droneLogError("ingest_pdf", str(e))
        raise HTTPException(
            status_code=500,
...
async def upload_csv(request: dict):
    ...
    try:
        ...
        # 라벨 안전 처리 (Cypher 인젝션 방지)
        safe_entity_type = "".join(c for c in str(entity_type) if c.isalnum() or c == "_")
        if not safe_entity_type:
            safe_entity_type = "Entity"
        ...
        for row in data:
            ...
            sanitized_rows.append(properties)

        # 배치 업로드
        query = f"UNWIND $rows AS row MERGE (n:{safe_entity_type} {{name: row.name}}) SET n += row"
        for i in range(0, len(sanitized_rows), batch_size):
            batch = sanitized_rows[i:i + batch_size]
            if not batch:
                continue
            db.execute_query(query, {"rows": batch})
        ...
        droneLogError("upload_csv", str(e))
...
async def upload_json(request: dict):
    ...
    try:
        ...
        droneLogError("upload_json", str(e))
        raise HTTPException(status_code=500, detail=str(e))
