...
class DataIntegrator:
    ...
    def ingestPdfEntities(self, entities: List[Dict[str, Any]], permanent: bool = False):
        """
        ...
        Args:
            ...
            permanent: If True, save to permanent database. If False, temporary (session only)
        """
        ...
        print(f"📚 Ingesting {len(entities)} entities from PDF ({storage_type})")
        ...
        for entity in entities:
            ...
            if permanent:
                ...
                # Temporary overlay node (do not overwrite permanent nodes)
                safe_label = "".join(c for c in str(entity_type) if c.isalnum() or c == "_")
                if not safe_label:
                    safe_label = "Entity"
                ...
                # Create temporary node only (no relationship)
                query = f"""
                MERGE (t:{safe_label} {{name: $name, temporary: true}})
                SET t += $properties
                RETURN t
                """

                with self.driver.session() as session:
                    session.run(query, name=str(name), properties=properties)
            
            self.stats['pdf_chunks'] += 1
        
        print(f"✅ Merged {self.stats['pdf_chunks']} PDF entities ({storage_type})")

    # Backward compatibility
    def ingest_pdf_entities(self, entities: List[Dict[str, Any]], permanent: bool = False):
        return self.ingestPdfEntities(entities=entities, permanent=permanent)
